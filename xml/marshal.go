...
func (p *printer) marshalValue(val reflect.Value, finfo *fieldInfo, startTemplate *StartElement) error {
	...
	if startTemplate != nil {
		...
		if xmlname.name != "" {
			start.Name.Space, start.Name.Local, start.Name.Prefix = xmlname.xmlns, xmlname.name, xmlname.prefix
		} else {
	...
	if start.Name.Local == "" && finfo != nil {
		start.Name.Space, start.Name.Local, start.Name.Prefix = finfo.xmlns, finfo.name, finfo.prefix
	}
	if start.Name.Local == "" {
		name := typ.Name()
		if name == "" {
	...
	for i := range tinfo.fields {
		...
		name := Name{Space: finfo.xmlns, Local: finfo.name, Prefix: finfo.prefix }
...
func defaultStart(typ reflect.Type, finfo *fieldInfo, startTemplate *StartElement) StartElement {
	...
	if startTemplate != nil {
		...
		start.Name.Prefix = finfo.prefix
	} else if typ.Name() != "" {
